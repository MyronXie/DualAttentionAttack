...
def run_cam(data_dir, epoch, train=True, batch_size=BATCH_SIZE):
    print(data_dir)
    dataset = MyDataset(data_dir, 224, texture_size, faces, vertices, distence=None, mask_dir=mask_dir)
    loader = DataLoader(
        dataset=dataset,     
    ...
    for _ in range(epoch):
        ...
        for i, (index, total_img, texture_img, masks) in enumerate(tqdm_loader):
            ...
            total_img_np.save(os.path.join(log_dir, 'total.png')) 

            Image.fromarray((255 * texture_img).data.cpu().numpy()[0].transpose((1, 2, 0)).astype('uint8')).save(os.path.join(log_dir, 'texture.png'))
